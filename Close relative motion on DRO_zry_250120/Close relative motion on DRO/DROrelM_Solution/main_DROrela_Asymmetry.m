...
%         X_Xr(7:12,:)=rel_motion_L;
        ...
        % 将绝对运动轨道转移至惯性系中 m
         ...
         for ii = 1:2000
%              Al(1:4,ii)=alpha(X_Xr(1:3,ii),X_Xr(4:6,ii),X_Xr(7:9,ii),X_Xr(10:12,ii),con.mu);
...
%             x_max = max(rel_motion_L_Nolinear(1,:));
%             x_min = min(rel_motion_L_Nolinear(1,:));
%             x_middle = (x_max+x_min)/2;
%             x_diff = x_max - x_min;
%             y_middle = (max(rel_motion_L_Nolinear(2,:))+min(rel_motion_L_Nolinear(2,:)))/2;
...
%   title('|a3+a21|');
%   subplot(4,2,7)
%   plot(t_sample/para.T0,Al(3,:)*1000) ;hold on
%   xlabel('\it相位 \rm[T]'); ylabel('\ita \rm[m/s2]')
%   title('|a1+a2+a3|');
%   subplot(4,2,8)
