...
function [Alpha]= alpha(r_C, v_C, r_REL, v_REL,mu)
...
Arho2=-(mu/r_LVLH3*(I-3*r_LVLH*r_LVLH'/r_LVLH2 )...
     + (1-mu)/r_E_LVLH3*(I-3*r_E_LVLH*r_E_LVLH'/r_E_LVLH2));
Arho=Arho1+Arho2;
...
% mi=norm(omega_mi,2);
% c=v_REL_dot
Alpha(1)= norm(-2*Omega_li*v_REL,2)/(norm(-2*Omega_li*v_REL,2)+norm(Arho1*r_REL,2)+norm(Arho2*r_REL,2));
Alpha(2) =norm(-2*Omega_li*v_REL,2);
Alpha(3) =norm(-2*Omega_li*v_REL,2)+norm(Arho1*r_REL,2)+norm(Arho2*r_REL,2);
Alpha(4) =norm(v_REL_dot);
