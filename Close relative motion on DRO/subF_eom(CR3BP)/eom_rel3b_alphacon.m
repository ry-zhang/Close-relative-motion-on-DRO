...
function ydot= eom_rel3b_alphacon(t,y,mu)
...
% a=piv(phidot)norm(omega_li);
% b=norm(omega_mi);
% c=v_REL_dot
% 
% Alpha=(norm(-2*Omega_li*v_REL)+norm(-Omega_li_dot*r_REL))/(norm(-2*Omega_li*v_REL)+norm(Arho1*r_REL)+norm(Arho2*r_REL));
