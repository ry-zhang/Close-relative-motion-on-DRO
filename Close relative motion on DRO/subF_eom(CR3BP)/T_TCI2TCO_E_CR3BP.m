function [rv_TCO_c] = T_TCI2TCO_E_CR3BP(rv_TCI_c,rv_ECI_t,t_sample,Flag_frame,mu)
% 主星原点惯性系(target-centered inertial,TCI)转化到轨道坐标系(target-centered orbital,TCO)
% v1 2020/8/20 fhl
% v2 2021/6/1 YCH
% v3 2021/8/20 YCH
...
for ii = 1:row
    r_TCI_c_ii = rv_TCI_c(ii,1:3); 
    if col == 3
        v_TCI_c_ii = [];
    else
        v_TCI_c_ii = rv_TCI_c(ii,4:6);
    end
    r_ECI_t_ii = rv_ECI_t(ii,1:3); v_ECI_ii = rv_ECI_t(ii,4:6);
    theta = t_sample(ii);
    [r_TC_c_ii,v_TC_c_ii] = TCI2TCO(r_TCI_c_ii',v_TCI_c_ii',r_ECI_t_ii',v_ECI_ii',theta,Flag_frame,mu);
    rv_TCO_c(ii,:) = [r_TC_c_ii',v_TC_c_ii'];
end

end

function [r_TCO_c,v_TCO_c] = TCI2TCO(r_TCI_c,v_TCI_c,r_ECI_t,v_ECI_t,theta,Flag_frame,mu)
...
%坐标转换
TM_TCI2TCO = [i_OinR j_OinR k_OinR]';
r_TCO_c = TM_TCI2TCO*r_TCI_c;

if isempty(v_TCI_c)
    v_TCO_c = [];
else
    TM_TCI2TCO_dot = [i_OinR_dot,j_OinR_dot,k_OinR_dot]';
    v_TCO_c = TM_TCI2TCO*v_TCI_c + TM_TCI2TCO_dot*r_TCI_c;
%     v_TCO_c = TM_TCI2TCO*v_TCI_c + cross(omegaT,r_TCO_c);
end
