...
%     end
    ...
    if svd2ndLast<aux.svdBurfTol  && aux.computeBurf &&  ~currBurf  && (iLoop>=aux.burfLoc ||   iLoop>=103)
...
%      set(gcf,'Resize','off');
     ...
     if isempty(ie)
         ...
         if iLoop > aux.burfLoc 
          ...
          if iLoop > 103
             plot3(xx_rot(: , 1) , xx_rot(: , 2) , xx_rot(: , 3) , 'g' , 'linewidth' , 0.2);
...
%写文件 
filePath = [pwd , '\Orbit_data\' , '3DResoOrb1_2type2'];
fid = fopen(filePath , 'a');
fprintf(fid , '%s   %s   %s   %s   %s   %s   %s  %s\n', 'X(LU)','Y(LU)','Z(LU)','Vx(VU)','Vy(VU)','Vz(VU)','T(TU)','C');
for ii = 103:size(dataSave,1)
