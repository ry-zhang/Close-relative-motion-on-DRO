            ...
            [233580], real_T posVel[6])
{
  ...
  d1 = dt / interval_length;
  b_fix(&d1);
  st.site = &c_emlrtRSI;
  getCoeff(&st, header[1 + (((int32_T)eObj - 1) << 2)], header[2 + (((int32_T)
             eObj - 1) << 2)], header[3 + (((int32_T)eObj - 1) << 2)], d1 + 1.0,
           PCtemp_data, PCtemp_size, coeff_data, coeff_size);
  st.site = &d_emlrtRSI;
  cheb_Interp(&st, coeff_data, coeff_size, 2.0 * (dt - ((d1 + 1.0) - 1.0) *
  ...
  if (eObj == 3.0) {
    ...
    b_fix(&d1);
    st.site = &e_emlrtRSI;
    b_getCoeff(&st, d1 + 1.0, PCtemp_data, PCtemp_size, coeff);
    st.site = &u_emlrtRSI;
    b_cheb_Interp(&st, coeff, 2.0 * (dt - ((d1 + 1.0) - 1.0) * 4.0) / 4.0 - 1.0,
                  rvMnGeo);
  ...
  if (eObj == 10.0) {
    ...
    b_fix(&d1);
    st.site = &f_emlrtRSI;
    c_getCoeff(&st, d1 + 1.0, PCtemp_data, PCtemp_size, coeff);
    st.site = &t_emlrtRSI;
    b_cheb_Interp(&st, coeff, 2.0 * (dt - ((d1 + 1.0) - 1.0) * 16.0) / 16.0 -
                  1.0, rvMnGeo);
