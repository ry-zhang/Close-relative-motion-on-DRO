% 给定星历下的一组时间，j2k状态(jdf,xf_j2k)，求其附近的周期相对运动在ECILVLH下的几何角度alpha_f(逆时针)
function alpha_f_deg = fit_xeci2alpha(jdf, xf_j2k)
...
% [fit_theta2alpha, gof_theta2alpha] = fit( theta_all, alpha_all, ft, opts_fit );


%% 计算初始状态在ECILVLH下的几何相位，以及ECILVLH下的目标几何相位的时间
r2d = 180/pi;
load('DE430Coeff.mat');%星历表
xf_MCR = T_ECJ2k2Rot(jdf, xf_j2k, [],DE430Coeff, 'MCEMR');
theta_f = mod(atan2(-xf_MCR(2),xf_MCR(1)),2*pi);
theta_f_shift = mod(theta_f - pi,2*pi);

alpha_f_shift = interp1(theta_all_shift,alpha_all_shift,theta_f_shift);
