function iaxes = MagInset(h, ax, ZoomArea, InsetPos, Lines)
    ...
    if(BadInput == 0)
        ...
        annotation('rectangle',[zax(1) zay(1) (zax(2) - zax(1)) (zay(2) - zay(1))],'LineWidth',0.5);
        ...
        if(nargin > 4)
            ...
            if((numLine>0) && (size(Lines,2) == 2))
                ...
                for ii=1:1:numLine
                    ...
                    % Add the line:
                    annotation('line', lx,ly,'LineWidth',0.5);
                end
            end
        end
        ...
        % Set the scale of the iaxes according to the MainAx
        iaxes.XScale = MainAx.XScale;
        iaxes.YScale = MainAx.YScale;
%         grid on;
        hold on;
        box on;
...
function [xn, yn] = xy2norm(axh, x, y)
    ...
    if strcmp(axh.XScale,'log')
        xlims = log10(xlims);
        x = log10(x);
    end
    if strcmp(axh.YScale,'log')
        ylims = log10(ylims);
        y = log10(y);
    end
