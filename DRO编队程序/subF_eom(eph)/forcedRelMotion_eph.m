function [x0f_MCR_target_maneuver,x0f_TCO_rel_maneuver,dv_TCO_all_km,xx_MCR_target_traj,xx_TCO_rel_traj,r_error,t_sample_traj] ...
...
% x_TCO_rel_km          [nx3]or[nx6]    副星的变轨点的状态(km)
%                                       [r0;r1;...;rn](位置/位置+速度均可),速度为变轨delta v提供初值
% dt_all                [(n-1)x1]       两次变轨之间的时间间隔(sec)
% Flag_coordinate       [char]          指定副星变轨点的坐标系('VVLH','VNC','LVLH')
...
%% 将变轨处的目标点转移至LVLH坐标系
if strcmp(Flag_coordinate,'LVLH')
    x_LVLH_rel_all = x_TCO_rel_km;
elseif strcmp(Flag_coordinate,'VNC') || strcmp(Flag_coordinate,'VVLH')
    ...
    x_LVLH_rel_all = T_TCR2TCO_eph(x_MCR_rel_all,x0f_MCR_target_maneuver,a0f_MCR_target_maneuver,'LVLH');
else
    error('Wrong frame');
end

num_dv = size(x_LVLH_rel_all,1)-1;
dv_LVLH_all_km = zeros(num_dv,3);
fval_all = zeros(num_dv,1);
x0f_LVLH_rel_AfterManeuver = zeros(num_dv+1,6);
x0f_LVLH_rel_BeforeManeuver = zeros(num_dv+1,6);
if isDisplay == 1
    fprintf('------- 星历下相对运动变轨优化 ---------\n')
end
aux.t0UTC = []; % 删除t0UTC以防止与jd0冲突
xx_LVLH_rel_traj_cell = cell(num_dv,1);
xx_MCR_chaser_traj_cell = cell(num_dv,1);
xx_MCR_target_traj_cell = cell(num_dv,1);
aa_MCR_target_traj_cell = cell(num_dv,1);
for ii_index = 1:num_dv
% parfor ii_index = 1:num_dv
    if isDisplay == 1
    ...
    dt = dt_all(ii_index);
    aux0 = aux;
    aux0.jd0 = aux.jd0 + t_all(ii_index) / 86400;
    % 设定初值
    if size(x_LVLH_rel_all,2)==6
        x0_LVLH_rel_loop = x_LVLH_rel_all(ii_index,1:6);
    else
        x0_LVLH_rel_loop = [x_LVLH_rel_all(ii_index,1:3),0,0,0];
    end
    x0 = [0,0,0];
    rf_LVLH_rel = x_LVLH_rel_all(ii_index+1,1:3);
    ...
    [dv,fval] = fsolve(@(x)maneuver_relmotion_eph_STM(x,dt,x0f_MCR_target,a0f_MCR_target,x0_LVLH_rel_loop,rf_LVLH_rel,aux0),x0,opts_fsolve);
...
%     [dv,fval] = fsolve_GN(@maneuver_relmotion_eph_STM,x0,paraIter,dt,x0f_MCR_target,a0f_MCR_target,x0_REL_loop,rf_REL,aux);
    ...
    x0_LVLH_rel_loop(4:6) = x0_LVLH_rel_loop(4:6)+dv;

    % chaser初值, LVLH 2 MCR
    x0_MCR_chaser = T_TCO2TCR_eph(x0_LVLH_rel_loop,x0_MCR_target_loop,a0f_MCR_target(1,:),'LVLH')+x0_MCR_target_loop;
    
    % 副星星历积分
    t_sample = linspace(0,dt,max(ceil(dt/3600/4)+1,40)); % 4个小时打一个点
    xx_MCR_chaser = Propagate_EphRotFrame(x0_MCR_chaser,[0 dt],t_sample,aux0);
    [xx_MCR_target_loop,aa_MCR_target_loop] = Propagate_EphRotFrame(x0_MCR_target_loop,[0 dt],t_sample,aux0);
    % chaser, MCR 2 LVLH
    xx_MCR_rel = xx_MCR_chaser-xx_MCR_target_loop;
    xx_LVLH_rel = T_TCR2TCO_eph(xx_MCR_rel,xx_MCR_target_loop,aa_MCR_target_loop,'LVLH');
    
    if norm(fval)>1e-3
%         error('优化未收敛，第%d次优化差%.1e km未到达目标点',ii_index,norm(fval))
        warning('优化未收敛，第%d次优化差%.1e km未到达目标点',ii_index,norm(fval))
    else
        if isDisplay == 1
    ...
    % 存储数据
    dv_LVLH_all_km(ii_index,:) = dv;
    fval_all(ii_index) = norm(fval);
    x0f_LVLH_rel_AfterManeuver(ii_index,:) = x0_LVLH_rel_loop;
    x0f_LVLH_rel_BeforeManeuver(ii_index+1,:) = xx_LVLH_rel(end,:);
    t_sample_cell{ii_index} = t_sample + (aux0.jd0 - aux.jd0)*86400;
    xx_MCR_chaser_traj_cell{ii_index} = xx_MCR_chaser;
    xx_MCR_target_traj_cell{ii_index} = xx_MCR_target_loop;
    aa_MCR_target_traj_cell{ii_index} = aa_MCR_target_loop;
    xx_LVLH_rel_traj_cell{ii_index} = xx_LVLH_rel;
    
    % 更新下一次变轨数据
%     x0_LVLH_rel_loop = xx_LVLH_rel(end,1:6);
...
aa_MCR_target_traj = [];
for ii_index = 1:num_dv
    t_sample_traj = [t_sample_traj,t_sample_cell{ii_index}];
    xx_MCR_chaser_traj = [xx_MCR_chaser_traj;xx_MCR_chaser_traj_cell{ii_index}];
    xx_MCR_target_traj = [xx_MCR_target_traj;xx_MCR_target_traj_cell{ii_index}];
    aa_MCR_target_traj = [aa_MCR_target_traj;aa_MCR_target_traj_cell{ii_index}];
    xx_LVLH_rel_traj = [xx_LVLH_rel_traj;xx_LVLH_rel_traj_cell{ii_index}];
end

x0f_LVLH_rel_AfterManeuver(num_dv+1,:) = xx_LVLH_rel_traj(end,1:6);
x0f_LVLH_rel_BeforeManeuver(1,:) = x0f_LVLH_rel_AfterManeuver(1,:) + [zeros(1,3),dv_LVLH_all_km(1,:)];
dv_LVLH_all_km = x0f_LVLH_rel_AfterManeuver(:,4:6) - x0f_LVLH_rel_BeforeManeuver(:,4:6);
...
if strcmp(Flag_coordinate,'VNC') || strcmp(Flag_coordinate,'VVLH')
    x0f_MCR_rel_maneuver = T_TCO2TCR_eph(x0f_LVLH_rel_AfterManeuver,x0f_MCR_target_maneuver,a0f_MCR_target_maneuver,'LVLH');
    x0f_TCO_rel_maneuver = T_TCR2TCO_eph(x0f_MCR_rel_maneuver,x0f_MCR_target_maneuver,a0f_MCR_target_maneuver,Flag_coordinate);
    xx_MCR_rel_traj = T_TCO2TCR_eph(xx_LVLH_rel_traj,xx_MCR_target_traj,aa_MCR_target_traj,'LVLH');
    xx_TCO_rel_traj = T_TCR2TCO_eph(xx_MCR_rel_traj,xx_MCR_target_traj,aa_MCR_target_traj,Flag_coordinate);
    dv_MCR_all_km = T_TCO2TCR_eph(dv_LVLH_all_km,x0f_MCR_target_maneuver,a0f_MCR_target_maneuver,'LVLH');
    dv_TCO_all_km = T_TCR2TCO_eph(dv_MCR_all_km,x0f_MCR_target_maneuver,a0f_MCR_target_maneuver,Flag_coordinate);
else
    x0f_TCO_rel_maneuver = x0f_LVLH_rel_AfterManeuver;
    xx_TCO_rel_traj = xx_LVLH_rel_traj;
    dv_TCO_all_km = dv_LVLH_all_km;
end
...
%     hold off
    ...
    p5 = plot3(xx_TCO_rel_traj(:,1),xx_TCO_rel_traj(:,2),xx_TCO_rel_traj(:,3),'Color',[0, 114, 189]/255,'LineWidth',1.5); 
    hold off
