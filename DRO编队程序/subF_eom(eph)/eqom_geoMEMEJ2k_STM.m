...
% --------------------- 功能--------------
% 行星际飞行运动积分
% 坐标系：geo MEME J2k STM，地心J2K平赤道平春分点坐标系(状态转移矩阵版）
%  主天体：地球
%  主要摄动：太阳、月球
...
for i = 1:length(mainBody)
    ...
    y = rp2sc(2,index);
    z = rp2sc(3,index);
    G(1,1) = G(1,1) + (-mu*r2 + 3*mu*x^2)/r5;
    G(2,2) = G(2,2) + (-mu*r2 + 3*mu*y^2)/r5;
    G(3,3) = G(3,3) + (-mu*r2 + 3*mu*z^2)/r5;
    G(1,2) = G(1,2) + 3*mu*x * y/r5;
    G(1,3) = G(1,3) + 3*mu*x * z/r5;
    G(2,3) = G(2,3) + 3*mu*y * z/r5;
end
G(2,1) = G(1,2);
G(3,1) = G(1,3);
G(3,2) = G(2,3);
